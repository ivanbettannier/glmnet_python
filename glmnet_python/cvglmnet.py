...
=======================
INPUT ARGUMENTS
 x           nobs x nvar np 2D array of x parameters (as in glmnet).
 y           nobs x nc np Response y as in glmnet.
...
def cvglmnet(*, x,
    ...
    if len(foldid) == 0:
        ...
        foldid = np.reshape(foldid, [foldid.size,])
    else:
    ...
    if ptype == 'auc':
        cvm = -cvm
    CVerr['lambda_min'] = np.amax(options['lambdau'][cvm <= np.amin(cvm)]).reshape([1])  
